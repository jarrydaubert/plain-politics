...
Politics Platform is a UK-focused, source-first political intelligence platform using Next.js 16 + Supabase + Vercel. When working on structured data for this project:

### Planned Implementation
- Schema component: shared `StructuredData` component rendering JSON-LD server-side
- All schemas use `<script type="application/ld+json">` in server-rendered HTML
- XSS protection: escape `</script>` in JSON output
- `Organization` schema emitted from root layout (appears on every page)

### Active Schema Types (Target)
- **Homepage**: `WebSite`, `Organization`, `SoftwareApplication`, `FAQPage`
- **Explainer pages** (`/explain/[topic]`): `Article`, `BreadcrumbList`, `FAQPage` (if Q&A content present)
- **Compare pages** (`/compare/[issue]`): `ItemList`, `BreadcrumbList`
...
- All monetary values in GBP where applicable
