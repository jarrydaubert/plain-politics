...
Can UK Policy Explainer turn a small set of public political sources into a useful, neutral, plain-English information tracker that people trust?
...
3. Two to three authoritative source families.
4. Party profile cards.
5. One Compare view.
6. Search and plain-English explainer pages over the ingested records.
7. One source panel.
8. One methodology page.
...
2. Party profile cards explaining each included party in plain English.
3. A comparison view showing party positions in the same structure.
4. Search and explainer pages that never make unsupported claims.
5. A source panel that shows exact source excerpts.
6. A source detail page or panel with publisher, URL, tier, retrieved date, published date, and freshness state.
...
6. The team can add a second issue area using the same pattern.
